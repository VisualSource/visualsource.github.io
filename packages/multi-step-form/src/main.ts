declare var Alpine: {
  data(key: string, callback: () => void): void;
  store(key: string, data: Record<string, unknown>): void;
  store<T>(key: string): T;
  bind(key: string, callback: () => unknown): void;
}
declare var Iodine: {
  assert: (value: unknown, rules: string[]) => { valid: boolean; rule: string; error: string; }
    ...
    return {
      ...
      submit(ev) {
        ...
        if ((ev.target as HTMLFormElement).checkValidity()) {
          Alpine.store<FormState>("formState").next();
        }
