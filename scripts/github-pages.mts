...
try {
    ...
    for (const project of await readdir(paths.INPUT.PROJECTS)) {
        ...
        links.push({
            templete: imageItem.replace("{{project_name}}", name)
    ...
    const file = html.replace("{{mainContent}}", imageContent)
        .replace("{{linksContent}}", linkContent);
